...
export default function Product({ product, setSelected }: Props) {
  ...
  return (
    <motion.div
      ...
      key={product.id}>
      <Image className='h-full max-h-[50%] object-contain' src={product.image} alt={product.title} width={200} height={200} />
      ...
      <div className='p-4 flex justify-between items-center bg-zinc-100 w-full mt-auto'>
        <p className='font-semibold text-lg'>{product.price.toLocaleString('en-us', { style: 'currency', currency: 'USD' })}</p>
        <button className='bg-black text-white p-4 rounded-lg' onClick={() => dispatch(addToCart(product))}>
          Add to Cart
        </button>
      </div>
    </motion.div>
